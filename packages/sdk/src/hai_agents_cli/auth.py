"""Browser-based portal sign-in: RFC 8252 loopback redirect + PKCE, then mint an API key.

Ported from holo-desktop. Opens the system browser to the portal, captures the
auth code on a localhost callback, exchanges it for a short-lived JWT, and mints
a per-machine API key scoped to the user's organization.
"""
...
_SUCCESS_HTML = (
    b"<!doctype html><title>hai</title><h2>Signed in. You can close this tab and return to your terminal.</h2>"
)
_ERROR_HTML = b"<!doctype html><title>hai</title><h2>Sign-in failed. Return to your terminal and try again.</h2>"


def _pkce_pair() -> tuple[str, str]:
    """RFC 7636 S256: a URL-safe verifier and its base64url(SHA-256) challenge."""
...
def login_and_mint(portal: str, label: str, on_open: typing.Callable[[str], None]) -> str:
    """Run the full sign-in, returning a freshly minted API key. ``on_open`` receives the authorize URL."""
    ...
    with httpx.Client(timeout=20.0) as client:
        ...
        minted = _mint_key(client, portal, org_id, label)
    return minted["key"]
