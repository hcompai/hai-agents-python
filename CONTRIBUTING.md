...
The Python source under `src/agent_platform/` is regenerated from
...
**Do not hand-edit anything under `src/`.** A sync PR will overwrite your
changes the next time the upstream schema moves. If you need to change client
behavior:
...
`src/` is generated but committed: each automated sync PR commits the
regenerated code so reviewers see the diff. The initial code lands via the
first sync PR after this repo is bootstrapped. CI regenerates `src/` from
`openapi.json` on every PR, so the test job always exercises a fresh build.

## Run tests

```bash
uv run pytest tests/                                  # unit (default; integration deselected)
uv run pytest tests/integration -m integration -v     # live API (needs HAI_API_KEY_TEST)
```
...
- [ ] `uv run pytest tests/ -m "not integration"` passes locally.
